...
| 问题类型 | 示例 | 为什么必须调用 |
|---------|------|--------------|
| 查询文件列表 | "我有哪些文件" | 文件列表在数据库中，你无法知道 |
| 查询收藏/星标 | "我收藏了什么" | 收藏状态是用户私有数据 |
| 存储统计 | "用了多少空间" | 存储用量是动态计算的 |
| 最近文件 | "最近上传了什么" | 需要查询时间戳排序 |
| 分享记录 | "分享了哪些文件" | 分享链接在数据库中 |
| 标签信息 | "有哪些标签" | 标签是用户自定义的 |
| 文件内容 | "这个文件写了什么" | 必须读取真实内容 |
| 任何涉及"多少/哪些/列表/统计"的问题 | - | 都需要查询实时数据 |

### 禁止行为清单（红线）：

❌ **绝对禁止**：
1. 编造文件名、文件数量、存储用量等数字
2. 猜测用户的收藏、标签、分享状态
3. 说"您有X个文件"、"使用了YGB空间"等具体数据
4. 列出你不确定是否存在的文件
5. 用模糊词汇掩盖编造行为（"大约"、"大概"、"可能有一些"）
6. 描述你要调用什么工具，但实际不调用

✅ **正确做法**：
1. 识别到需要查询数据 → **立即调用对应工具**
2. 收到工具返回的真实数据 → **基于数据回答**
3. 工具返回空结果 → **诚实告知用户未找到**
...
❌ **错误（编造数据）**：
> "您一共收藏了5个项目，包括OSSshelf项目文件夹和工作备忘录等文件..."
> *(这是严重的错误！你在编造数据)*

❌ **错误（描述但不执行）**：
> "让我使用 get_starred_files 工具来帮您查询收藏的文件。这个工具会返回您的收藏列表..."
> *(这是无效的！你说要调用但没调用)*

✅ **正确（调用工具）**：
> *(直接输出 tool_call 代码块，不解释)*
> \`\`\`tool_call
> {"name": "get_starred_files", "arguments": {}}
> \`\`\`

**如果你发现自己在直接回答任何数据类问题，立即停止！改为调用相应工具！**

**记住：宁可说"让我查询一下"然后真正调用工具，也不要编造任何数据！**`;
...
export class AgentEngine {
  ...
  async run(
    ...
    const result = await this.runPromptBased(userId, query, conversationHistory, modelId, caps, config, onChunk, signal, sessionId, filteredTools, contextPrompt);
    return {
      ...result,
      meta: result.meta ?? { toolCallCount: 0, modelId: resolvedModelId, inputTokens: 0, outputTokens: 0 },
    };
    ...
    while (toolCallCount < config.maxToolCalls) {
      ...
      try {
        ...
        if (isExpectedAbort(err, hasToolCalls)) {
          ...
          if (toolCallCount === 0) {
            ...
            return this.runPromptBased(userId, query, conversationHistory, modelId, caps, config, onChunk, signal, sessionId, filteredTools);
          } else {
      ...
      if (!hasToolCalls) {
        ...
        if (shouldHaveCalledTool && toolCallCount === 0) {
          ...
          return this.runPromptBased(userId, query, conversationHistory, modelId, caps, config, onChunk, signal, sessionId, filteredTools, contextPrompt);
        }
...
function detectMissingToolCall(query: string, responseText: string): string | undefined {
  ...
  // 快速检查1：如果回复中包含真实的文件/文件夹引用，说明已经调用了工具
  const hasRealDataMarkers = /\[FILE:|\[FOLDER:|https?:\/\/[^\s]+\.(pdf|doc|txt|jpg|png)/i.test(responseText);

  // 快速检查2：如果回复很短（<50字符），可能是简单确认或错误信息
  if (responseText.length < 50) {
    return undefined;
  }

  // 快速检查3：如果回复明确表示无法完成或需要更多信息，不应该强制调用工具
  const isRefusalOrClarification = /无法|不能|需要(更多|具体|额外)信息|请提供|不清楚|不知道|抱歉/i.test(lowerResponse);
  if (isRefusalOrClarification && !hasRealDataMarkers) {
    return undefined; // 模型在合理地请求澄清或表示无法完成
  }

  // 如果有真实数据标记且回复长度适中，可能是正常响应
  if (hasRealDataMarkers && responseText.length < 800) {
    return undefined;
  }

  // 遍历所有工具关键词映射，找出最匹配的工具
  let bestMatch: { toolName: string; confidence: number } | null = null;

  for (const [toolName, patterns] of Object.entries(TOOL_KEYWORD_MAP)) {
    // 检查用户查询是否匹配该工具的意图
    const queryMatches = patterns.some(pattern => pattern.test(lowerQuery));

    if (!queryMatches) continue;

    // 检查模型回复是否提到了相关内容但缺乏真实数据
    const responseMentionsTopic = patterns.some(pattern => pattern.test(lowerResponse));

    if (!responseMentionsTopic) continue;

    // 计算置信度（基于多个维度的综合判断）
    let confidence = 0;

    // 维度1：回复长度（中等权重）
    // 较长的回复更可能在编造数据，但也不能太绝对
    if (responseText.length > 150) confidence += 1;
    if (responseText.length > 400) confidence += 1;

    // 维度2：包含具体的数字+单位组合（高风险指标）
    // 但排除常见的数量词如"1个"、"2个"等小数字
    const hasSpecificNumbers = /\d{2,}(\.\d+)?\s*(个|项|文件|GB|MB|KB|TB|%|次|字节)/i.test(responseText);
    if (hasSpecificNumbers) confidence += 3;

    // 维度3：模糊词汇（中等风险指标）
    const vagueWords = /大约|大概|可能|估计|around|about|一些|多个/i.test(lowerResponse);
    if (vagueWords) confidence += 1;

    // 维度4：提到要执行动作但无实际执行证据（高风险指标）
    const mentionsAction = /让我|我来|帮您|我将|可以.*(查看|查询|搜索|找|列出|获取)/i.test(lowerResponse);
    if (mentionsAction && !hasRealDataMarkers) confidence += 2;

    // 维度5：列举了具体项目但无真实引用（调整后的权重）
    // 只有当列举的项目数>=3且无真实数据时才算高风险
    const listItems = responseText.match(/(\d+\.|-|\*)\s*[^\n]+/g);
    if (listItems && listItems.length >= 3 && !hasRealDataMarkers) {
      confidence += 2; // 从+4降到+2，减少误判
    }

    // 维度6（新增）：模型明确提到工具名但没有tool_call格式（极高风险）
    const mentionsToolByName = new RegExp(`\\b${toolName.replace('_', '[ _-]')}\\b`, 'i').test(lowerResponse);
    if (mentionsToolByName && !hasRealDataMarkers) {
      confidence += 3;
    }

    // 提高阈值到8，确保只有高置信度才触发（从5提升到8）
    if (confidence >= 8 && (!bestMatch || confidence > bestMatch.confidence)) {
